...
class STGNNLayer(nn.Module):
    ...
    def __init__(
        ...
        if self._gcn_true:
            self._graph_conv = Diff_GCN(
                conv_channels, conv_channels, gcn_depth, dropout, support_len=1
            )

        self._batchnorm = nn.BatchNorm2d(conv_channels)
        if self._readout == "conv":
            self.end_conv = nn.Conv2d(
                in_channels=conv_channels, out_channels=1, kernel_size=(1, 1), bias=True
            )
        else:
    ...
    def forward(
        ...
        if self._readout == "conv":
            ...
            X_pool = torch.mean(
                X, -1
            )  # Temporal pooling, output shape (B,C,N)  # Reconsider temporal pooling after mean and max
