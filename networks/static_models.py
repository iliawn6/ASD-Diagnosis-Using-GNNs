...
class GIN(Abstract_GNN):
    def __init__(self, num_nodes, f1, f2, readout, extra_fc, **kwargs):
        ...
        self.conv1 = GINConv(
            Sequential(
                Linear(num_nodes, f1), BatchNorm1d(f1), ReLU(), Linear(f1, f1), ReLU()
            )
        )

        self.conv2 = GINConv(
            Sequential(Linear(f1, f2), BatchNorm1d(f2), ReLU(), Linear(f2, f2), ReLU())
        )

        last_dim = 2 if readout == "meanmax" else 1
        if self.ll:
            self.last = Sequential(
                Linear(f2 * last_dim, f2 * last_dim * 2),
                ReLU(),
