...
class Abide(Dataset):
    ...
    def __init__(self, df_dir, th, train, use_gdc=False):
        ...
        sample_file = data_info["tc_file"].iloc[0].replace("ATLAS", "HO_sub")
        ...
        for i, sub_i in enumerate(data_info.index):
            ...
            tc_file = data_info["tc_file"].iloc[i].replace("ATLAS", "HO_sub")
            tc_vals = pd.read_csv(tc_file).values.transpose()
            if tc_vals.shape[1] < self.ntime:
                tc_vals = np.array(
                    [
                        np.pad(
                            tc_vals[i], (0, self.ntime - tc_vals.shape[1]), "reflect"
                        )
            ...
            cc_file = data_info["cc_file"].iloc[i].replace("ATLAS", "HO_sub")
            ...
            rows, columns = prop_thresholding(
                cc_vals, th
            )  # np.where(np.abs(cc_vals)>th)
            edge_inds = torch.tensor(
                np.column_stack((rows, columns)).transpose(), dtype=torch.long
            )
